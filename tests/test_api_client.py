...
@pytest.mark.asyncio
async def test_get_torrent_prefers_torrent_file(monkeypatch: Any) -> None:
    async def fake_search(
        query: str, sources: list[str] | None = None
    ) -> list[Torrent]:
        return []
    ...
    torrent.torrent_file = "/tmp/example.torrent"
    torrent.prepend_info("file query", 10)
    api.CACHE.update([torrent])
    assert await api.get_torrent(torrent.id) == "/tmp/example.torrent"
